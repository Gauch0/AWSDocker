...
docker run -d --name api -p 1000:3000 -e DB_HOST=$DB_HOST -e DB_USER=$DB_USER -e DB_PASS=$DB_PASS -e DB_NAME=$DB_NAME -d api

docker run -d --name ui -p 80:8000 -e BACKEND_URL=$BACKEND_URL -d ui